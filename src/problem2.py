...
def problem2a(circle, rectangle, window):
    ...
    crl = rg.Circle(circle.center,circle.radius)
    crl.attach_to(window)
    crl.fill_color = circle.fill_color
    ...
    up = rec.get_upper_right_corner()
    down = rec.get_lower_left_corner()

    line = rg.Line(up,down)
    line.arrow = 'last'
    ...
    crl.fill_color = rectangle.outline_color
    window.render()
...
def problem2b(rect, n, delta, win):
    ...
    left = rect.get_upper_left_corner()
    right = rect.get_lower_right_corner()

    first = rg.Rectangle(left,right)
    first.fill_color = rect.fill_color
    first.attach_to(win)
    ...
    for k in range(n - 1):
        ...
        right.y = right.y + (delta)
        after = rg.Rectangle(left,right)
        after.attach_to(win)
